...
def pipes2(playerX, playerY, x10, y10, speed):
    gameExit = False
  ...
  # State 1 means the initial state or freely falling state (Not flapping state)
    ...
    while not gameExit:
     ...
     # Calling pipe function for two pipes for now (both on the ground)
        ...
        # For pipe above
        if not gameExit and gameStarted == True:
            x10, y10, gameExit = pipes2(playerX, playerY, x10, y10, speed)
        if not gameExit and gameStarted == True:
            x102, y102, gameExit = pipes2(playerX, playerY, x102, y102, speed)

...
def pipes1(playerX, playerY, x1, y1, speed, isEaten):
    gameExit = False
    x1 += speed
...
def powerComing(playerX, playerY, powerBallX, powerBallY, powerSpeed, gameStarted, farFromEdge, isEaten, lastingPower):
    ...
    # If powerball hasn't been eaten, we will determine whether it is eaten or not
    if isEaten == False:
        # Below code is to ensure the collision of bird and powerball (aka it is eaten)
        if ((powerBallX + 30 < playerX + 100 < powerBallX + 60) and (powerBallY + 30 < playerY + 90 < powerBallY + 60)
            ...
            isEaten = True
            # To set time of powerball since it is eaten
    ...
    while not gameExit:
     ...
     # Calling pipe function for two pipes for now (both on the ground)
        ...
        if not gameExit and gameStarted == True:
            x1, y1, gameExit = pipes1(playerX, playerY, x1, y1, speed, isEaten)
        if not gameExit and gameStarted == True:
            x2, y2, gameExit = pipes1(playerX, playerY, x2, y2, speed, isEaten)
        # For pipe above
        if not gameExit and gameStarted == True:
